package de.peekandpoke.kraft.addons.forms.semanticui.input

import de.peekandpoke.kraft.addons.semanticui.forms.input.InputFieldComponent
import de.peekandpoke.kraft.addons.utils.*
...
fun Tag.InputField(
    ...
    InputFieldComponent.Config(
        value = value, type = InputType.number, onChange = onChange, toStr = ::numberToString, fromStr = ::stringToDouble
    ).apply(configure)
        .let { config -> comp(config.asProps) { ctx -> InputFieldComponent(ctx) } }
